# Supabase Queries & Setup Guide

## 1. Initial Setup (Run in Supabase SQL Editor)

These queries set up the database schema, including tables for collections and contact messages.
...
## 2. Seed Data (Insert Initial Content)

Run this to populate your `collections` table with the initial portfolio items.

```sql
insert into public.collections (title, category, description, long_description, features, image, images, is_featured)
values
  (
    'Raw Denim Series',
    'Denim',
...
```sql
select * from public.collections where is_featured = true;
```

### Update a Collection to be Featured
```sql
update public.collections 
set is_featured = true 
where title = 'Vintage Selvedge';
```

### Delete a Message
```sql
delete from public.contacts where email = 'spam@example.com';
```
